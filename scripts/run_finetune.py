...
def train(args, train_dataset, model, tokenizer):
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate,
                      eps=args.adam_epsilon, betas=(args.beta1, args.beta2))
...
def load_and_cache_examples(args, task, tokenizer, split):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        # FIX: make get_test_examples
        examples = getattr(processor, f"get_{split}_examples")()
        # examples = (
        ...
        if args.n_process == 1:
            ...
            for i in range(n_proc):
                results.append(p.apply_async(convert_examples_to_features, args=(
                    examples[indexes[i]:indexes[i+1]], tokenizer, max_length, None, label_list, output_mode, pad_on_left, pad_token, pad_token_segment_id, True,)))
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, split="test")
    ...
    #                 args, model, tokenizer, prefix=prefix, split="test")
    #         elif args.result_dir.split('/')[-1] == "train.npy":
    #             results, eval_task, _, out_label_ids, probs = evaluate(
    #                 args, model, tokenizer, prefix=prefix, split="train")
    #         else:
