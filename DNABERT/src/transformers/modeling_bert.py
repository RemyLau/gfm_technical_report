...
@add_start_docstrings(
    """Bert Model transformer feature extractor.""",
    BERT_START_DOCSTRING,
)
class BertFeatureExtractor(BertPreTrainedModel):
    ...
    def forward(
        ...
        Returns:
            :obj:`tuple(torch.FloatTensor)` comprising pooled and averaged embedding from the input sequence:
            pooled_emb (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, config.hidden_size)`):
                Pooled embedding using the first token.
            avg_emb (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, config.hidden_size)`):
                Average embedding of the whole sequence.
        ...
        seq_embs, pooled_emb, *_ = outputs

        # Average of sequence embeddings
        avg_emb = ((seq_embs * attention_mask[:, :, None]).sum(1) / attention_mask.sum(1, keepdim=True))

        return pooled_emb, avg_emb
